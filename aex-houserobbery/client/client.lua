...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(450, function()
        ...
        Citizen.Wait(250)
        Framework.Functions.TriggerCallback("aex-houserobbery:server:get:config", function(ConfigData)
            Config = ConfigData
        end)
        LoggedIn = true
    end) 
...
RegisterNetEvent('aex-police:SetCopCount')
AddEventHandler('aex-police:SetCopCount', function(Amount)
    CurrentCops = Amount
end)

-- Code

RegisterNetEvent('aex-houserobbery:client:set:door:status')
AddEventHandler('aex-houserobbery:client:set:door:status', function(RobHouseId, bool)
    Config.HouseLocations[RobHouseId]['Opened'] = bool 
end)

RegisterNetEvent('aex-houserobbery:client:set:locker:state')
AddEventHandler('aex-houserobbery:client:set:locker:state', function(RobHouseId, LockerId, Type, bool)
    Config.HouseLocations[RobHouseId]['Lockers'][LockerId][Type] = bool 
end)

RegisterNetEvent('aex-houserobbery:client:set:extra:state')
AddEventHandler('aex-houserobbery:client:set:extra:state', function(RobHouseId, Id, bool)
    Config.HouseLocations[RobHouseId]['Extras'][Id]['Stolen'] = bool 
end)

RegisterNetEvent('aex-houserobbery:server:reset:state')
AddEventHandler('aex-houserobbery:server:reset:state', function(RobHouseId)
    Config.HouseLocations[RobHouseId]['Opened'] = bool 
        ...
        if LoggedIn then
            ...
            for k, v in pairs(Config.HouseLocations) do
                ...
                if Distance < 2.0 then 
                  ...
                  if not ShowingItems and not v['Opened'] then
                    ShowingItems = true
                    TriggerEvent('aex-inventory:client:requiredItems', ItemsNeeded, true)
                  end
                end
            end
            if not NearRobHouse then
                if ShowingItems then
                    ShowingItems = false
                    TriggerEvent('aex-inventory:client:requiredItems', ItemsNeeded, false)
                end
                Citizen.Wait(1500)
...
RegisterNetEvent('aex-items:client:use:lockpick')
AddEventHandler('aex-items:client:use:lockpick', function(IsAdvanced)
 local PlayerCoords = GetEntityCoords(GetPlayerPed(-1))
  Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
    if CurrentHouse ~= nil then
        if CurrentCops >= Config.CopsNeeded then
            if IsAdvanced then
               exports['aex-lockpick']:OpenLockpickGame(function(Success)
                if Success then
                    LockpickFinish(true)
                else
                    if math.random(1,100) < 19 then
                        TriggerServerEvent('Framework:Server:RemoveItem', 'advancedlockpick', 1)
                        TriggerServerEvent('aex-police:server:CreateBloodDrop', GetEntityCoords(GetPlayerPed(-1)))
                        TriggerEvent("aex-inventory:client:ItemBox", Framework.Shared.Items['advancedlockpick'], "remove")
                        Framework.Functions.Notify("You pricked your finger with your lockpick, your finger is dripping with blood", "error")
                    end
                end
               end)
            else
                   ...
                   exports['aex-lockpick']:OpenLockpickGame(function(Success)
                    if Success then
                        ...
                        if math.random(1,100) <= 35 then
                          ...
                          TriggerServerEvent('aex-police:server:CreateBloodDrop', GetEntityCoords(GetPlayerPed(-1)))
                          TriggerEvent("aex-inventory:client:ItemBox", Framework.Shared.Items['lockpick'], "remove")
                          Framework.Functions.Notify("You pricked your finger with your lockpick, your finger is dripping with blood", "error")
                        end
                    end
                   end)
               else
                   Framework.Functions.Notify("You are missing a toolkit..", "error")
               end
            end
        else
...
function LockpickFinish(Success)
 if Success then
   ...
   LockpickAnim(Time)
   Framework.Functions.Progressbar("lockpick-door", "Open Door..", Time, false, true, {
       disableMovement = true,
       disableCarMovement = true,
       disableMouse = false,
       disableCombat = true,
   }, {}, {}, {}, function() -- Done    
       TriggerServerEvent('aex-houserobbery:server:set:door:status', CurrentHouse, true)
       EnterHouseRobbery()
    ...
    Framework.Functions.Notify("You failed..", "error")
 end
end
...
function EnterHouseRobbery()
    ...
    TriggerEvent("aex-sound:client:play", "house-door-open", 0.1)
    OpenDoorAnim()
    InsideHouse = true
    Citizen.Wait(350)
    if math.random(1, 100) <= 36 then
        local StreetLabel = Framework.Functions.GetStreetLabel()
        TriggerServerEvent('aex-police:server:send:house:alert', GetEntityCoords(GetPlayerPed(-1)), StreetLabel)
    end
    if Config.HouseLocations[CurrentHouse]['Tier'] == 1 then
        HouseInterior = exports['aex-interiors']:HouseRobTierOne(CoordsTable)
    elseif Config.HouseLocations[CurrentHouse]['Tier'] == 2 then
        HouseInterior = exports['aex-interiors']:HouseRobTierOne(CoordsTable)
    else
        HouseInterior = exports['aex-interiors']:HouseRobTierThree(CoordsTable)
    end
    TriggerEvent('aex-weathersync:client:DisableSync')
    TriggerEvent("aex-sound:client:play", "house-door-close", 0.1)
    ...
    if Config.HouseLocations[CurrentHouse]['HasDog'] ~= nil and Config.HouseLocations[CurrentHouse]['HasDog'] then
        exports['aex-assets']:RequestModelHash("A_C_Rottweiler")
...
function LeaveHouseRobbery()
    ...
    exports['aex-interiors']:DespawnInterior(HouseData, function()
      ...
      TriggerEvent('aex-weathersync:client:EnableSync')
      DoScreenFadeIn(1000)
      CurrentHouse = nil
...
function StealPropItem(Id)
   ...
   TriggerServerEvent('aex-houserobbery:server:recieve:extra', CurrentHouse, Id)
end

function OpenLocker(LockerId)
  local Time = math.random(15000, 18000)
  if not IsWearingHandshoes() then
    TriggerServerEvent("aex-police:server:CreateFingerDrop", GetEntityCoords(GetPlayerPed(-1)))
  end
  LockpickAnim(Time)
  TriggerServerEvent('aex-houserobbery:server:set:locker:state', CurrentHouse, LockerId, 'Busy', true)
  Framework.Functions.Progressbar("lockpick-locker", "Search..", Time, false, true, {
    disableMovement = true,
    disableCarMovement = true,
    disableMouse = false,
    disableCombat = true,
    }, {}, {}, {}, function() -- Done    
      TriggerServerEvent('aex-houserobbery:server:locker:reward', math.random(1,3))
      TriggerServerEvent('aex-houserobbery:server:set:locker:state', CurrentHouse, LockerId, 'Busy', false)
      TriggerServerEvent('aex-houserobbery:server:set:locker:state', CurrentHouse, LockerId, 'Opened', true)
      StopAnimTask(GetPlayerPed(-1), "veh@break_in@0h@p_m_one@", "low_force_entry_ds", 1.0)
    end, function() -- Cancel
...
function LockpickAnim(time)
  time = time / 1000
  exports['aex-assets']:RequestAnimationDict("veh@break_in@0h@p_m_one@")
  ...
  Citizen.CreateThread(function()
      while OpeningSomething do
          TriggerServerEvent('aex-hud:server:gain:stress', 1)
...
function OpenDoorAnim()
 exports['aex-assets']:RequestAnimationDict('anim@heists@keycard@')
